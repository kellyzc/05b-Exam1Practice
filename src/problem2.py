...
def problem2b(rect, n, delta, win):
    ...
    rect.attach_to(win)
    for i in range(n):
        rectangle = rg.Rectangle(rg.Point(rect.get_upper_left_corner().x - delta * i, rect.get_upper_left_corner().y - delta * i),
                                rg.Point(rect.get_lower_right_corner().x + delta * i, rect.get_lower_right_corner().y + delta * i))
